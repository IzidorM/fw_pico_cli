...
Call the `pico_cli_run` function periodically in the main loop task to operate the Pico CLI.
...
Rewrite the upper text to be more suitable for a readme.md file on my github repo.
...
Entire implementation is condensed into a single `pico_cli.c` file, accompanied by two header files `pico_cli.h` and `pico_cli_internal.h`.

These defines can be altered prior to compilation:

- CLI_COMMAND_BUFF_SIZE: It defines the maximum input size of prompt input (default size is 32 bytes).
- PICO_CLI_USE_STATIC_MEMORY_ALLOCATION: Use static memory allocation.
...
Compile with `PICO_CLI_USE_STATIC_MEMORY_ALLOCATION` defined.
...
Below is an example of how to define commands and incorporate them into your CLI interface:
...
Unit test files are stored in the `unit_test` folder. Before running these tests, make sure to update the path to Unity in the Makefile. The test should operate smoothly on any Linux system with installed GCC.
